...
public class TokenController {
    ...
    public ResultBean<JSONObject> getToken(@PathVariable String username, String password) {
        JSONObject loginInfo = mallUserService.login(username, password);
        if (loginInfo != null) {
            return new ResultBean<>(HttpStatusCode.OK.value(), HttpStatusCode.OK.getReasonPhrase(), loginInfo);
        } else {
